...
- **Tedim hymns are SEEDED, not bundled.** `tools/seed_tedim_hymns.py` collects
  the Tedim hymnal on YOUR server at deploy time — the exact pattern
  seed_hymns.py already uses for Open Hymnal. Each entry carries the Tedim
  title, the English original it translates (the mood-tagging signal), the
  verses, and the page's YouTube embed id when present (~80%+ of pages).
...
workers/
  ...
  tools/seed_tedim_hymns.py   SEEDS the Tedim library (run once per deploy)
...
python workers/tools/seed_tedim_hymns.py     # Tedim hymnal (~470 entries, polite delay)
