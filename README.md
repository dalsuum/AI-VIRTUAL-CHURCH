...
| [bible_router.py](workers/bible_router.py) | Read-only FastAPI routes (`/bible/books`, `/bible/chapter`, `/bible/languages`) backing the online Bible reader. Serves the same in-memory public-domain translations as `bible_api`; Laravel proxies these under `/api/bible`. |
...
| [BibleReader.vue](frontend/src/components/BibleReader.vue) | Online Bible reader at `#bible`. Browse books/chapters in English (BSB), Burmese (Judson 1835) and Tedim (Lai Siangtho 1932); switching translation keeps your place. Reads via the public `/api/bible/*` proxy (`BibleController`) — no auth required. |
