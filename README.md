...
| [ZolaiVocabulary.vue](frontend/src/components/ZolaiVocabulary.vue) | Searchable **multilingual** reference at `#vocabulary` (nav label "Vocabulary") with a **comparative-language dropdown** (a column chooser across the Chin/Zo tongues + Burmese/Hebrew/English, not a UI-locale selector). Its **default primary column now follows the one global language** (English→English, မြန်မာ→Burmese, Zolai→Zolai); the dropdown still lets the reader compare Falam, Hakha, Matu, Mizo, Paite, Sizang. The chosen language becomes the left-most column; English/Hebrew/Burmese stay as reference columns (the picked language is not repeated). Loads **live** from the public `GET /api/vocabulary` endpoint — the `vocabularies` DB table is the source of truth, edited from the **Admin Console → Vocabulary** tab. The original `frontend/src/data/zolai_vocabulary.json` is now only the one-time **seed** source (`VocabularySeeder`), not read at runtime. Search matches every language + notes (Hebrew renders right-to-left); the back link collapses to an icon on phones. |
...
**Single language authority (Phase 1).** The application has exactly **one** language
...
truth). Changing the global language instantly drives **Worship Radio** (`POST /music/recommend`
language), **Bible Reader** (opens the matching translation — registry → available versions →
the worshipper's remembered preferred edition per language) and **Vocabulary** (default primary
column) with no page reload. Locale string files live in
[frontend/src/i18n/locales/](frontend/src/i18n/locales/) — `en.json` is authored; the other 13
are scaffolded and fall back to English per key until translated. *(Phase 1 scope: Worship
Radio, Bible Reader, Vocabulary, and the locale scaffold. Pastor Chat and Church Service still
carry their own pickers — collapsed in Phase 2. Admin tools keep their per-language config
selectors, which configure backend per-language settings, not the worshipper's UI.)* The
architectural invariant going forward: **never introduce another language state** — every
worshipper-facing component derives language from this one authority.
