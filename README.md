...
| [BibleReader.vue](frontend/src/components/BibleReader.vue) | Online Bible reader at `#bible`. Browse books/chapters in English (BSB), Burmese (Judson 1835) and Tedim (Lai Siangtho 1932); switching translation keeps your place. A **🔊 Listen** button narrates the open chapter aloud — English via the admin's configured voice provider, Burmese & Tedim via the native MMS-TTS voices — reusing the service narration stack, and **highlights each verse as it's read** (proportional-timing, like the service player). An admin can also set an optional **looping background-music track** played softly behind the narration (`bible_bg_music_url` + `bible_bg_music_volume`); the music element starts/pauses/stops in lockstep with the spoken audio and is off when no URL is set. The Bible voice (per language), the highlight toggle, and the background-music URL/volume are set in **Admin Console → Settings → "Bible reader voice"** (`bible_narration_mode_*`, `bible_text_highlight_enabled`, `bible_bg_music_*`), independent of the live-service narrator but inheriting the voice when unset. Reads via the public `/api/bible/*` proxy (`BibleController`, incl. `/bible/config`) — no auth required. |
