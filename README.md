...
| [tools/seed_language_data.py](workers/tools/seed_language_data.py) | One-time seeder: downloads the English KJV (`data/kjv.json` — from dalsuum/bible if present, else built via `build_kjv_bible.py`), Judson 1835 (Myanmar) and Lai Siangtho 1932 (Tedim) Bibles, the Hebrew Tanakh (WLC, `data/wlc.json` — from dalsuum/bible if present, else built via `build_hebrew_bible.py`), the book index, and Myanmar hymns into `workers/data/`. |
