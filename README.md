...
| [tedim_router.py](workers/tedim_router.py) | FastAPI router: `POST /tedim/translate`, `POST /tedim/generate`, `GET /tedim/verse?ref=`. Redis db 2 cache (30-day TTL). Single-inference semaphore. `_validate_tedim()` enforces strict coherence checks: minimum length, requires ≥2 sentence-final `hi`/`hen` particles in terminal position, rejects sentence-initial `hi`/`hen` (word salad marker), and rejects repeated-word token shuffling — returns HTTP 502 on failure so `llm_engine` falls back to handcrafted Tedim content instead of serving word salad. Default system prompt now includes full opening-prayer and benediction templates for the 1B model to pattern-match against. |
