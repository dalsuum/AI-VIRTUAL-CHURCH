...
if not api and not brw:
    print("=== PENDING: no cycles recorded yet — waiting for the first cron tick ===")
    sys.exit(0)
has_fail = any(not r.get("ok") for r in api) or any(not r.get("ok") for r in brw)
print("=== " + ("ATTENTION: failures recorded — inspect logs" if has_fail else "ALL GREEN") + " ===")
