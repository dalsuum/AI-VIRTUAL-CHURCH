...
from tasks import orchestrate, repair_missing_narration, study_discuss

r = redis.from_url(os.getenv("REDIS_URL", "redis://localhost:6379/0"))


def main() -> None:
    print("Bridge consumer listening on ai:intake, ai:narration-repair, ai:study ...")
    while True:
        queue, raw = r.blpop(["ai:intake", "ai:narration-repair", "ai:study"])
        try:
            job = json.loads(raw)
